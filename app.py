...
class CertModel(db.Model):
    __tablename__ = 'TRUWORTHS'
    __table_args__ = TABLE_ARGS
    id = db.Column('ID',db.Integer, primary_key=True)
    phonenumber = db.Column('PHONENUMBER',db.String(12))
    emailaddress = db.Column('EMAILADDRESS',db.String(100))
...
#retrieve records with phonenumber 
@app.get('/TRUWORTHS/phonenumber/<string:phonenumber>')
@app.output(CertOutSchema)
@app.auth_required(auth)
@app.input(CertQuerySchema, 'query')
def get_phonenumber(phonenumber, query):

    pagination = CertModel.query.filter(CertModel.phonenumber == phonenumber).paginate(
        page=query['page'],
        per_page=query['per_page']
    )
    def get_page_url(page):
        return url_for('get_phonenumber', phonenumber=phonenumber, page=page, per_page=query['per_page'], _external=True)
    ...
    table_html = "<table border='4'><tr><th>Phone number</th><th>Email</th></tr>"

    # Add each valid certification to the table
    for cert in certs_data['certs']:
         table_html += f"<tr><td>{html.escape(cert.phonenumber)}</td>" \
              f"<td>{html.escape(str(cert.emailaddress))}</td></tr>"
